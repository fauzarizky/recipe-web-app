...
export const Bookmark = () => {
  const savedData = JSON.parse(localStorage.getItem("savedRecipe")) || [];
  ...
  return (
    <Box bgColor={"#F7F7F7"} w={"100vw"} h={"100vh"}>
      <header className="px-5 py-3">
        <Heading fontSize={"xl"}>Bookmark</Heading>
      </header>
      ...
      <main className="px-5">
        ...
        <Box display={"flex"} flexDirection={"column"} gap={2} alignItems={"center"} h={"67vh"} w={"100%"} overflowY={"auto"} borderRadius={"lg"} pr={2} transitionDuration={"0.7s"}>
          {savedData.length > 0 ? (
            filteredData.map((meal) => (
              <Box key={meal.idMeal} display={"flex"} gap={2} alignItems={"center"} borderRadius={"md"} h={"20vh"} w={"100%"} bgColor={"white"}>
                <Box w={"50%"} h={"100%"}>
            ...
            <Text>No saved recipe</Text>
          )}
        </Box>
        <DeleteConfirmationModal isOpen={showDeleteConfirmationModal} onClose={() => setShowDeleteConfirmationModal(false)} selectedRecipe={selectedRecipe} deleteFor={"bookmark"} />
      </main>

      <footer className="px-5">
        <Navigation activePage={2} />
      </footer>
    </Box>
  );
};
